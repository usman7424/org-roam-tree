...
using both =org-roam-tree-backlinks-section= and =org-roam-tree-reflinks-section= at the same time is *currently broken*.

You can customize =org-roam-tree-collapse-after-init= to enable/disable auto-collapse after building the buffer. 

* TODOs:

** [X] remember & restore collapsed states
** [X] remove vertical bar  on final file
** [X] fix wrapping next header when collapsing a note
